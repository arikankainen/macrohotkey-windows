...
**_MouseScroll(Y)_**\
Vierittää hiiren rullaa `Y` kertaa. `Y`:n määrä riippuu hiiren asetuksista. Positiivinen arvo vierittää ylöspäin, negatiivinen alaspäin.
...
**_StartAgain()_**\
Hyppää makron alkuun ja ajaa sen uudelleen. Tästä tulee ikuinen looppi, kunnes käyttäjä keskeyttää sen painamalla `Pause/Break`.
